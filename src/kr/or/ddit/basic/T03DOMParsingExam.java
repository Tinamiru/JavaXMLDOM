...
public class T03DOMParsingExam {
    public void parse() throws Exception {
        ...
        URL url = new URL("http://211.237.50.150:7080/openapi/" + apiKey + "/xml/" + svcKey + "/" + startIdx + "/" + endIdx + "?RECIPE_ID=" + recipeId);
        ...
        url = new URL("http://211.237.50.150:7080/openapi/" + apiKey + "/xml/" + svcKey + "/" + startIdx + "/" + endIdx + "?RECIPE_ID=" + recipeId);
        ...
        if (code.equals("INFO-000")) {
            ...
            for (int i = 0; i < rowNodeList.getLength(); i++) {
                ...
                String irdntNum = element.getElementsByTagName("IRDNT_NM").item(0).getTextContent();
                String irdntCpctyNum = element.getElementsByTagName("IRDNT_CPCTY").item(0).getTextContent();
                String irdntTyNm = element.getElementsByTagName("ITDNT_TY_NM").item(0).getTextContent();

                String str = String.format("%3s %8s %10s %10s %8s", rowNum, irdntNum, irdntCpctyNum, irdntTyNm);
